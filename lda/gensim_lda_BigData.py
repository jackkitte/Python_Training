...
def extractiveKeyword(text):
    ...
    f_wakati = open('dataset/books/xx01.txt.tag', 'rb')
    stopwords = f_stop.read()
    wakati_kaki = f_wakati.readlines()
    ...
    verb = "動詞"
    adjective = "形容詞"
    for line in wakati_kaki:
        feature = line.split()
        feature_word_class = feature[1].split('-')
        feature_uni = feature[0].decode('utf-8', 'ignore')
        print(feature_uni.encode('shift-jis', 'ignore'))
